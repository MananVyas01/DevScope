...
class GitLogParser:
    ...
    def get_recent_commits(self) -> List[Dict]:
        ...
        try:
            ...
            commit_iter = self.repo.iter_commits(
                since=since_time, max_count=50  # Limit to prevent excessive API calls
            )
            ...
            print(
                f"📋 Found {len(commits)} commits in the last {self.hours_back} hours"
            )
            return commits
    ...
    def _calculate_diff_stats(self, diff) -> Dict:
        ...
        stats = {"additions": 0, "deletions": 0, "files_changed": len(diff)}
    ...
    def _classify_commit_type(self, message: str) -> str:
        ...
        if any(word in message_lower for word in ["fix", "bug", "error", "issue"]):
            ...
            word in message_lower for word in ["feat", "feature", "add", "implement"]
        ):
            return "feature"
        elif any(
            word in message_lower for word in ["refactor", "restructure", "reorganize"]
        ):
            return "refactor"
