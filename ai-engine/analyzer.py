...
class CommitAnalyzer:
    ...
    def __init__(self, api_key: str, max_requests_per_minute: int = 3, monthly_budget: float = 5.0):
        """
        ...
        # Load cache
        self.cache_file = os.getenv('OFFLINE_CACHE_FILE', './cache/offline_analysis.json')
        self.cache = self._load_cache()
        
        print(f"🧠 CommitAnalyzer initialized with {max_requests_per_minute} req/min limit")
    ...
    def _build_analysis_prompt(self, commit_data: Dict) -> str:
        ...
        # Build file list
        file_list = ', '.join([f['path'] for f in files_changed[:10]])  # Limit to 10 files
...
+    setValue(prevValue => input)
   ...
   return <div>{value}</div>''',
            'files_changed': [{'path': 'src/components/Timer.tsx', 'change_type': 'modified'}],
