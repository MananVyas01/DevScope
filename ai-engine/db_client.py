...
class BackendAPIClient:
    ...
    def __init__(self, base_url: str, api_key: Optional[str] = None):
        ...
        # Offline storage
        self.offline_file = os.getenv('OFFLINE_CACHE_FILE', './cache/offline_analysis.json')
        self.retry_queue = []
    ...
    def _prepare_activity_data(self, commit_data: Dict) -> Dict:
        ...
        tags.extend([f'ext-{ext}' for ext in list(file_extensions)[:5]])  # Limit to 5 extensions
    ...
    async def retry_offline_data(self) -> Dict[str, int]:
        ...
        try:
            ...
            print(f"📊 Retry results: {stats['successful']}/{stats['attempted']} successful")
            return stats
        ...
        try:
            ...
            async with session.get(f"{self.base_url}/health") as response:
                if response.status == 200:
                    ...
                    print(f"✅ Backend health check passed: {health_data.get('status', 'unknown')}")
                    return True
                else:
