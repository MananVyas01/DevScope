...
async def main():
    ...
    try:
        ...
        def mock_analysis(commit):
            ...
            if "fix" in message or "bug" in message:
                return {
                    "label": "Bug Fixing",
                    "confidence": 0.85,
                    "reasoning": "Commit message indicates bug fixing",
                }
            elif "refactor" in message or "clean" in message:
                return {
                    "label": "Refactoring",
                    "confidence": 0.82,
                    "reasoning": "Code improvement and refactoring",
                }
            elif "feat" in message or "add" in message:
                return {
                    "label": "High Focus",
                    "confidence": 0.88,
                    "reasoning": "New feature development",
                }
                ...
                return {
                    "label": "Documentation",
                    "confidence": 0.83,
                    "reasoning": "Documentation updates",
                }
            else:
                return {
                    "label": "General Development",
                    "confidence": 0.75,
                    "reasoning": "General development work",
                }
        ...
        for i, commit in enumerate(commits, 1):
            ...
            try:
                if use_mock:
                    ...
                    print(
                        f"  ✅ Mock classified as: {analysis_result['label']} (confidence: {analysis_result['confidence']:.2f})"
                    )
                else:
                    ...
                    if analysis_result:
                        print(
                            f"  ✅ AI classified as: {analysis_result['label']} (confidence: {analysis_result['confidence']:.2f})"
                        )
                    else:
        ...
        print(
            f"✅ Successfully stored {success_count}/{len(analyzed_commits)} analyses"
        )
