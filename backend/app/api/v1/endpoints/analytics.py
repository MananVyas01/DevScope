...
async def get_dashboard_analytics(
    ...
    # Recent mood
    recent_mood_stmt = select(Mood).where(
        Mood.user_id == current_user.id
    ).order_by(Mood.created_at.desc()).limit(1)
...
async def get_productivity_trends(
    ...
    ).group_by(func.date(Mood.created_at)).order_by(func.date(Mood.created_at))
    ...
    return {
        ...
        "summary": {
            "total_days": days,
            "avg_daily_minutes": sum(p["total_minutes"] for p in productivity_data) / days if productivity_data else 0,
            "most_productive_day": max(productivity_data, key=lambda x: x["total_minutes"])["date"] if productivity_data else None,
        }
    }
...
async def get_project_breakdown(
    ...
    for project in projects:
        project["percentage"] = round((project["minutes"] / total_minutes * 100), 1) if total_minutes > 0 else 0
