...
async def get_mood_entry(
    ...
    stmt = select(MoodModel).where(
        and_(MoodModel.id == mood_id, MoodModel.user_id == current_user.id)
    )
...
async def get_mood_trends(
    ...
    if not moods:
        return {"trends": [], "averages": None, "insights": [], "total_entries": 0}
        ...
        trends.append(
            {
                "date": date.isoformat(),
                "avg_mood": round(
                    sum(data["mood_scores"]) / len(data["mood_scores"]), 2
                ),
                "avg_energy": round(
                    sum(data["energy_levels"]) / len(data["energy_levels"]), 2
                ),
                "avg_stress": round(
                    sum(data["stress_levels"]) / len(data["stress_levels"]), 2
                ),
    ...
    if len(trends) >= 7:
        ...
        earlier_mood = (
            sum(trend["avg_mood"] for trend in trends[-14:-7]) / 7
            if len(trends) >= 14
            else recent_mood
        )
        ...
        if recent_mood > earlier_mood + 0.5:
            ...
            insights.append(
                "Your mood has been trending downward lately. Consider what might be affecting it. 📉"
            )
        else:
    ...
    if low_energy_days > len(trends) * 0.3:  # More than 30% low energy days
        insights.append(
            "You've had quite a few low-energy days. Make sure you're getting enough rest! ⚡"
        )
    ...
    if high_stress_days > len(trends) * 0.3:  # More than 30% high stress days
        insights.append(
            "You've been experiencing high stress levels frequently. Consider stress management techniques. 🧘‍♀️"
        )

    # Consistency insights
    if len(moods) < days * 0.5:  # Less than 50% tracking consistency
        insights.append(
            "Try to log your mood more consistently for better insights! 📊"
        )
    else:
...
async def get_mood_correlations(
    ...
    for activity in activities:
        ...
        if date_key in daily_data:
            ...
            daily_data[date_key]["activity_types"][
                activity_type
    ...
    if len(mood_scores) >= 5:
        ...
        if len(set(productive_days)) > 1 and len(set(mood_scores)) > 1:
            try:
                ...
                if correlation > 0.3:
                    insights.append(
                        "Higher mood scores correlate with more productive days! 🎯"
                    )
                elif correlation < -0.3:
                    insights.append(
                        "Interestingly, your mood doesn't seem to directly correlate with productivity."
                    )
                else:
                    insights.append(
                        "Your mood and productivity show a mild correlation."
                    )
    ...
    for activity_type, correlations_data in activity_mood_correlation.items():
        if len(correlations_data) >= 3:
            avg_mood_for_activity = sum(d["mood"] for d in correlations_data) / len(
                correlations_data
            )
    ...
    if good_mood_activities:
        ...
        insights.append(
            f"You tend to be happiest when doing {best_activity['activity']} activities! 😊"
        )
...
async def mood_quick_check(
    ...
    recent_stmt = (
        select(MoodModel)
        .where(MoodModel.user_id == current_user.id)
        .order_by(MoodModel.created_at.desc())
        .limit(5)
    )
