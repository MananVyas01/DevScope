        ...
        if recent_commits:
            ...
            for commit in recent_commits:
                ...
                if (streak_date - commit_date).days <= 1:
                    if commit_date == streak_date or commit_date == streak_date - timedelta(days=1):
...
async def sync_github_activity(
    ...
    for commit in commits:
        ...
        activity_stmt = select(Activity).where(
            ...
            Activity.description.contains(commit.sha[:8])  # Check if commit SHA is in description
        )
        ...
        if not existing_activity:
            ...
            estimated_minutes = min(max((commit.additions + commit.deletions) // 10, 15), 120)
